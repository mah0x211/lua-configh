...
lua-configh is a lightweight autoconf alternative that generates config.h files. It checks for the availability of C headers, functions, types, declarations, struct members, and type sizes by compiling test code with the C compiler.
...
```bash
configh <config.lua> [--out=<filename>] [--help]
```
...
return {
    -- C compiler name (optional, default: $CC; error if neither is set)
    cc = 'gcc',
    ...
    --
    -- when checking headers, functions, types, declarations, type sizes, and members.
    output_status = true,
...
check decl: POSIX_VERSION ... not found
  ...
  >  1 error generated.
  >   
check header: stddef.h ... found
check sizeof: size_t ... found (8)
check sizeof: ptrdiff_t ... found (8)
...
/* Define to 1 if you have the <stddef.h> header. */
